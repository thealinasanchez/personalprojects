...
function handleSignup() {
    ...
    .then(response => {
        ...
        // fetch the user ID associated with the created username
        return fetch("http://localhost:8080/usernames?username=" + encodeURIComponent(username));
    })
    .then(response => {
        if (!response.ok) {
...
function generateFolders(folderData) {
    ...
    folderEditIcon.id = "folder-edit-icon";
    folderEditIcon.className = "icon";
    ...
    folderDeleteIcon.id = "folder-delete-icon";
    folderDeleteIcon.alt = "Delete icon";
    folderDeleteIcon.className = "icon";
    ...
    createFlashcardsButton.id = "create-flashcards-button";
    createFlashcardsButton.className = "button";
    ...
    folderArrow.id = "folder-arrow";
    folderDiv.appendChild(folderArrow);

    // flashcard div
    var flashcardDiv = document.createElement("div");
    flashcardDiv.className = "flashcard-container";
    folderDiv.appendChild(flashcardDiv);

    // event listener for clicking edit folder button
    folderEditIcon.addEventListener("click", function(event) {
        // prevent the click event from bubbling to the folder
        event.stopPropagation();
        editFolder(folderData.id);
    });

    // event listener for clicking delete folder button
    ...
    // event listener for create flashcards button
    createFlashcardsButton.addEventListener("click", function(event) {
        event.stopPropagation();
        createFlashcard(folderData.id);
    })

    // event listener to display flashcards when folder is clicked
    folderDiv.addEventListener("click", function() {
        var flashcardContainer = folderDiv.querySelector(".flashcard-container");
        if (flashcardContainer.innerHTML.trim() === "") {
            loadFlashcardsFromServer(folderData.id, flashcardContainer);
            if (loadFlashcardsFromServer) {
                document.getElementById("folder-arrow").src = "icons/up-arrow.png";
            } else {
                document.getElementById("folder-arrow").src = "icons/down-arrow.png";
            }
        } else {
            // clear flashcards if already loaded
...
function loadFoldersFromServer() {
    ...
    fetch(`http://localhost:8080/usernames/${userId}/folderlist`)
    .then(response => {
        if (!response.ok) {
            throw new Error("Network response was not ok");
        }
    ...
    .then(folderIds => {
        // clear the wrapper before appending new folders
        simpleCardsWrapper.innerHTML = "";
