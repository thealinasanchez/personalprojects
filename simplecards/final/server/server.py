...
@app.route('/usernames/<int:user_id>/folderlist', methods=["GET"])
def get_user_folderlist(user_id):
    db = Usernames("usernames.db")

    # fetch folder list associated with the user ID
    folder_list = db.getFolderlist(user_id)
    if isinstance(folder_list, list):

        # return the folder list as JSON response
        return jsonify(folder_list), 200
    else:
        return jsonify({"error": folder_list}), 404
...
def add_to_folders():
    try:
        ...
        db.createFolder(folder_name, creator_id)
        return "Created", 201, {"Access-Control-Allow-Origin":"*"}
...
@app.route("/folders/<int:folder_id>/flashcards", methods=["POST"])
def add_flashcard_to_folder(folder_id):
    try:
        db = Folders("folders.db")

        if "flashcard_id" not in request.form:
            return "Missing required field: flashcard_id", 400, {"Access-Control-Allow-Origin":"*"}
        
        flashcard_id = int(request.form["flashcard_id"])

        result, status_code = db.addFlashcardToFolder(folder_id, flashcard_id)
        return result, status_code, {"Access-Control-Allow-Origin":"*"}
...
def retrieve_flashcards_in_folder(folder_id):
    try:
        db = Folders("folders.db")
        flashcards = db.getFlashcardsInFolder(folder_id)
        return jsonify(flashcards), 200, {"Access-Control-Allow-Origin": "*"}
    except Exception as e:
        return f"An error occurred: {str(e)}", 500, {"Access-Control-Allow-Origin":"*"}
